...
- 인라인 수식이 숫자로 시작하면서 \`^ _ \\\\ { }\` 같은 LaTeX 연산자가 전혀 없는 순수 숫자식(예: 1 + 2 = 3)이면, 가격 표기와 혼동되어 렌더되지 않으므로 인라인 \`$...$\` 대신 독립된 줄의 블록 \`$$...$$\` 로 감싸세요(예: $$1 + 2 = 3$$).`;
...
- 선택지가 필요한 질문(판별/분류 등 객관식)은 선택지를 q 문장 안에 나열하지 말고 choices 배열(2-4개)로 분리하세요. q 에는 질문 문장만 남깁니다.
- choices 의 각 항목은 선택지 텍스트만 씁니다. "(a)", "1.", "가)" 같은 라벨/번호를 붙이지 마세요. 화면이 1, 2, 3 번호를 자동으로 붙입니다.
- 서술형 질문의 choices 는 null 로 두세요. 객관식은 질문 세트당 1-2개면 충분합니다.
...
3) 마커 다음 줄부터 questions 를 JSON 배열로만 출력합니다. 코드펜스(\`\`\`)나 설명문 없이 순수 JSON 만. 예: [{"id":"1-1","q":"..."},{"id":"1-2","q":"...","choices":["...","...","..."]}]
...
- 다음 4유형을 단계의 성격에 맞게 2유형 이상 섞으세요. 모두 한 유형으로 채우지 마세요:
  ...
  4) 반례 만들기: "X 가 아닌 예를 들어보세요"
- 질문 문장은 짧고 명료하게. 정답을 본문에서 그대로 베끼면 답할 수 있는 질문은 피하세요.
- 선택지가 필요한 질문(판별/분류 등 객관식)은 선택지를 q 문장 안에 나열하지 말고 "choices" 배열(2-4개)로 분리하세요. q 에는 질문 문장만 남깁니다. 예: {"id":"2-2","q":"다음 중 X 에 해당하지 않는 것은?","choices":["...","...","..."]}
- choices 의 각 항목은 선택지 텍스트만 씁니다. "(a)", "1.", "가)" 같은 라벨/번호를 붙이지 마세요. 화면이 1, 2, 3 번호를 자동으로 붙입니다.
- 서술형 질문에는 choices 키를 넣지 마세요. 객관식은 질문 세트당 1-2개면 충분합니다.
